...
### OBS 권장 인코딩 설정 (실측으로 확정 — 운영 송출 PC에 동일 적용)

HLS 재생 품질·싱크를 위해 아래를 맞춘다. OBS 자체 미리보기는 멀쩡해도 HLS로 나갈 때
문제가 생기는 항목들이라 반드시 확인:

- **키프레임 간격 = 2초** (설정 → 출력 → 출력 모드 "고급" → 방송 → 키프레임 간격).
  0(자동)이거나 크면 HLS 세그먼트가 어긋나 **A/V 싱크가 틀어진다**. 반드시 2로.
- **캔버스 해상도 : 출력 해상도 = 같은 비율(16:9)** (설정 → 비디오). 비율이 다르면
  OBS가 위/아래(또는 좌/우)로 레터박스를 **구워서** 송출 → 웹 플레이어에 검은 띠.
  예: 기본 1920×1080, 출력 1280×720.
- (선택) 미세 싱크: 남은 오디오 선행이 거슬리면 OBS **오디오 믹서 → 고급 오디오 속성 →
  동기화 오프셋(ms)** 에 양수(예: 300)로 오디오를 살짝 지연.
- HLS 특성상 ±수백 ms 오차·10~20초 지연은 정상. 재생 기기(폰) 기준으로 맞추면 됨.
